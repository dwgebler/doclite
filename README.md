...
> :warning: The cache is not invalidated by saving or data changing in storage, each 
> entry in the cache is only cleared or regenerated by either expiring through 
> the configurable cache lifetime, or by being manually flushed through a call 
> to `clearCache()`.
